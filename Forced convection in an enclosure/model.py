...
class mdPINN:
    ...
    def __init__(self, xy_l, xy_r, xy_x, xy_y, xy_f, xy_m, layers, layers1):
        ...
        self.x_x_tf = tf.placeholder(tf.float32, shape=[None, self.x_x.shape[1]]) 
        self.y_x_tf = tf.placeholder(tf.float32, shape=[None, self.y_x.shape[1]])   
        
        self.x_y_tf = tf.placeholder(tf.float32, shape=[None, self.x_y.shape[1]]) 
        self.y_y_tf = tf.placeholder(tf.float32, shape=[None, self.y_y.shape[1]])   
        ...
        self.T_pred_l = self.net_T(self.x_l_tf, self.y_l_tf) 
        self.T_pred_y = self.net_T(self.x_y_tf, self.y_y_tf) 
        ...
        self.fx, self.T_xx, self.T_yx = self.net_f(self.x_x_tf, self.y_x_tf) 
        self.fy, self.T_xy, self.T_yy = self.net_f(self.x_y_tf, self.y_y_tf) 
        ...
        self.loss_1 = tf.reduce_mean(tf.square(0.1*self.T_xl + 5))
        
        self.loss_2 = tf.reduce_mean(tf.square(0.1*self.T_xr + 0.5))
        
        self.loss_3 = tf.reduce_mean(tf.square(self.T_pred_m))
        
        self.loss_4 = tf.reduce_mean(tf.square(self.T_xy))
    ...
    def train(self,nIter):
        ...
        tf_dict = {self.x_f_tf: self.x_f, 
                   ...
                   self.x_x_tf: self.x_x, 
                   self.y_x_tf: self.y_x,   
                   self.x_y_tf: self.x_y, 
                   self.y_y_tf: self.y_y,   
    ...
    def predict(self, X_star1):
        ...
        tf_dict1 = {self.x_l_tf: X_star1[:,0:1], self.y_l_tf: X_star1[:,1:2]}     
        
        T_star = self.sess.run(self.T_pred_l, tf_dict1)   
