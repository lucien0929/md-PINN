...
class mdPINN:
    ...
    def __init__(self, xy_l, xy_r, xy_b, xy_m, xy_f, layers, layers1):
        ...
        self.T_pred_u = self.net_T(self.x_u_tf, self.y_u_tf) 
        self.T_pred_b = self.net_T(self.x_b_tf, self.y_b_tf) 
        self.T_pred_m = self.net_T(self.x_m_tf, self.y_m_tf) 
 
        self.f, self.T_xf, self.T_xf = self.net_f(self.x_f_tf, self.y_f_tf) 
        
        self.fu, self.T_xu, self.T_yu = self.net_f(self.x_u_tf, self.y_u_tf) 
        ...
        self.loss_1 = tf.reduce_mean(tf.square(self.T_pred_l - 50))
        
        self.loss_2 = tf.reduce_mean(tf.square(self.T_pred_m + 10))
        
        self.loss_3 = tf.reduce_mean(tf.square(self.T_yb))
        
        self.loss_4 = tf.reduce_mean(tf.square(self.T_pred_r + 10))
